...
from switchmap.dashboard.table.interfaces import InterfaceTable, InterfaceRow, table, _RawCol
...
class TestRawCol(unittest.TestCase):
    ...
    def test_td_format(self):
        ...
        self.assertEqual(Markup(result), Markup(html_content))  # Compare as Markup objects
        ...
        class MockInterface:
            ...
            def row(self):
                if not self._interface:
                    return None
                Row = namedtuple('Row', [
                    'port', 'vlan', 'state', 'days_inactive',
                    'speed', 'duplex', 'label', 'trunk', 'cdp',
                    'lldp', 'mac_address', 'organization',
                    'ip_address', 'hostname'
                ])
                return Row(
