...
class TestConfigHelperFunctions(unittest.TestCase):
    ...
    def test_mask_secrets_dict(self):
        ...
        config = {
            ...
            "nested": {"snmp_authpassword": "auth_secret", "regular": "value"},
        }
        result = mask_secrets(config)
        self.assertEqual(
            result["db_pass"], {"isSecret": True, "value": PLACEHOLDER}
        )
        self.assertEqual(
            result["snmp_community"], {"isSecret": True, "value": PLACEHOLDER}
        )
        self.assertEqual(result["normal_key"], "normal_value")
        self.assertEqual(
            result["nested"]["snmp_authpassword"],
            {"isSecret": True, "value": PLACEHOLDER},
        )
    ...
    def test_mask_secrets_list(self):
        ...
        config = [{"db_pass": "secret"}, {"normal": "value"}]
        result = mask_secrets(config)
        self.assertEqual(
            result[0]["db_pass"], {"isSecret": True, "value": PLACEHOLDER}
        )
...
class TestConfigFileOperations(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_config = {
            "server": {"db_host": "localhost", "db_pass": "secret_password"},
            "polling": {"interval": 300},
        }

    @patch(
        "switchmap.server.api.routes.config.CONFIG_PATH",
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("yaml.safe_load")
    def test_read_config_yaml_error(
    ...
    def test_write_config_success(
        ...
        mock_replace.assert_called_once_with(
            "/tmp/.config.tmp123", "/tmp/test_config.yaml"
        )
    ...
    @patch("os.fdopen")
    @patch("os.unlink")
    def test_write_config_exception_cleanup(
        self, mock_unlink, mock_fdopen, mock_mkstemp, mock_makedirs
    ):
...
class TestConfigAPIRoutes(unittest.TestCase):
    ...
    def test_get_config_success(self, mock_read_config):
        ...
        self.assertEqual(
            data["server"]["db_pass"], {"isSecret": True, "value": PLACEHOLDER}
        )
    ...
    def test_post_config_success(self, mock_write_config, mock_read_config):
        ...
        mock_read_config.return_value = self.test_config
        new_config = {
            "server": {"db_host": "newhost", "db_pass": "new_password"}
        }
    ...
    @patch("switchmap.server.api.routes.config.read_config")
    @patch("switchmap.server.api.routes.config.write_config")
    def test_patch_config_db_pass_success(
        self, mock_write_config, mock_read_config
    ):
        """Test successful PATCH /config for db_pass."""
        mock_read_config.return_value = self.test_config

        patch_data = {"db_pass": {"new": "updated_password"}}
    ...
    def test_patch_config_db_pass_invalid_format(self, mock_read_config):
        ...
        patch_data = {"db_pass": "invalid_format"}
    ...
    def test_patch_config_db_pass_missing_new(self, mock_read_config):
        ...
        patch_data = {"db_pass": {"old": "old_password"}}
    ...
    @patch("switchmap.server.api.routes.config.read_config")
    @patch("switchmap.server.api.routes.config.write_config")
    def test_patch_config_db_pass_empty_value(
        self, mock_write_config, mock_read_config
    ):
        """Test PATCH /config with empty db_pass value."""
        mock_read_config.return_value = self.test_config

        patch_data = {"db_pass": {"new": ""}}
    ...
    @patch("switchmap.server.api.routes.config.read_config")
    @patch("switchmap.server.api.routes.config.write_config")
    def test_patch_config_db_pass_placeholder_value(
        self, mock_write_config, mock_read_config
    ):
        """Test PATCH /config with placeholder db_pass value."""
        mock_read_config.return_value = self.test_config

        patch_data = {"db_pass": {"new": PLACEHOLDER}}
    ...
    @patch("switchmap.server.api.routes.config.read_config")
    @patch("switchmap.server.api.routes.config.write_config")
    def test_patch_config_other_fields(
        self, mock_write_config, mock_read_config
    ):
    ...
    @patch("switchmap.server.api.routes.config.read_config")
    @patch("switchmap.server.api.routes.config.write_config")
    def test_patch_config_no_server_section(
        self, mock_write_config, mock_read_config
    ):
        """Test PATCH /config when current config has no server section."""
        config_without_server = {"polling": {"interval": 300}}
        mock_read_config.return_value = config_without_server

        patch_data = {"db_pass": {"new": "new_password"}}
    ...
    def test_patch_config_no_data(self):
        ...
        response = self.client.patch(
            "/config", data="", content_type="application/json"
        )
