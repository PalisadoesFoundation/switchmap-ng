        ...
        if re.match(r'^\s*def ', line):
            ...
            if docstring_start < len(lines) and lines[docstring_start].strip().startswith('"""'):
                ...
                if docstring_end < len(lines):
                    ...
                    print(f"Extracted docstring from lines {docstring_start + 1} to {docstring_end + 1}")
                    ...
                    try:
                        ...
                        if parsed.returns and parsed.returns.type == "None" and parsed.returns.description == "None":
                            violations = [v for v in violations if v.get("issue") != "Missing 'Returns' section"]
                        if all(p.type == "None" and p.description == "None" for p in parsed.params):
                            violations = [v for v in violations if v.get("issue") != "Missing 'Args' section"]
